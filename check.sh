...
#git clone https://github.com/catonrug/projectlibre-detect.git && cd projectlibre-detect && chmod +x check.sh && ./check.sh
...
filelist=$(wget -qO- "$download" | sed "s/http/\nhttp/g;s/download/download\n/g" | grep "^http.*\/download$" | grep -v "txt\|dmg\|rpm\|tar\|deb\|README" | sort | uniq | sed '$alast line')
...
https://drive.google.com/drive/folders/0B_3uBwg3RcdVOC1laHBXOTJIeEE 
"
} done
echo
